...
class ViewcontrollerScore: ViewController{
    ...
    func getAllFIRData() {
        ...
        let Roomname:String = UserHomeViewController.Channelname
        if (Roomname.isEmpty == false){
            let databaseRef = Database.database().reference()

            databaseRef.child("Ranking").queryOrdered(byChild:"\(Roomname)").observeSingleEvent(of: .value, with: { snapshot in
                
                if snapshot.exists(){
                    self.create = Database.database().reference().child("Ranking").child("\(Roomname)/User")
                        ...
                        if let snapShort = snapshot.children.allObjects as? [DataSnapshot]{
                            for snap in snapShort{
                                if let mainDict = snap.value as? [String: AnyObject]{
                                    ...
                                    self.arrData.append(RankScoree(name: name!, gameSum: gameSum))
...
extension ViewcontrollerScore: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.nameLabel.text = ("Group: \(String(describing: test.name!))")
